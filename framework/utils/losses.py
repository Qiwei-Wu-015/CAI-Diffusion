...
class DiffusionLoss:
    def __init__(self,
        ...
        self.emotion_prediction_loss = EmotionPredictionLoss(w_pred=w_pred, w_au=w_au, w_va=w_va, w_em=w_em)
    ...
    def __call__(self, output_decoder):
        ...
        # # Reshape back for temporal/CCC calculation: [batch_size * n_preds, window_size, emotion_dim]
        # pred_seq = prediction_emotion.view(-1, window_size, emotion_dim)
        # target_seq = target_emotion.view(-1, window_size, emotion_dim)

        # if self.w_ccc > 0:
        #     loss_ccc = self.ccc_loss(pred_seq, target_seq)
        #     losses_dict["loss_ccc"] = loss_ccc
        #     losses_dict["loss"] += self.w_ccc * loss_ccc
            
        # if self.w_vel > 0:
        #     loss_vel = VelocityLoss(pred_seq, target_seq)
        #     losses_dict["loss_vel"] = loss_vel
        #     losses_dict["loss"] += self.w_vel * loss_vel

        # Calculate prediction loss if available
...
class EmotionPredictionLoss(nn.Module):
    ...
    def forward(self, pred_future, target_future):
        # pred_future: (batch_size, future_frames, emotion_dim)
        # target_future: (batch_size, future_frames, emotion_dim)
        # print(f"DEBUG: pred_future shape: {pred_future.shape}")
        # print(f"DEBUG: target_future shape: {target_future.shape}")
        ...
        loss_mse = loss_au * self.w_au + loss_va * self.w_va + loss_em * self.w_em
        loss = loss_mse
            
        return loss * self.w_pred
...
class CCCLoss(nn.Module):
    ...
    def forward(self, x, y):
        # x, y: [batch_size, seq_len, features]
        # Compute over the sequence dimension (dim=1)
        
        vx = x - torch.mean(x, dim=1, keepdim=True)
        vy = y - torch.mean(y, dim=1, keepdim=True)
        
        rho = torch.sum(vx * vy, dim=1) / (torch.sqrt(torch.sum(vx ** 2, dim=1)) * torch.sqrt(torch.sum(vy ** 2, dim=1)) + self.eps)
        
        x_mean = torch.mean(x, dim=1)
        y_mean = torch.mean(y, dim=1)
        x_var = torch.var(x, dim=1, unbiased=False)
        y_var = torch.var(y, dim=1, unbiased=False)
        x_std = torch.std(x, dim=1, unbiased=False)
        y_std = torch.std(y, dim=1, unbiased=False)
        
        # CCC = 2*rho*std_x*std_y / (var_x + var_y + (mean_x - mean_y)^2)
        numerator = 2 * rho * x_std * y_std
        denominator = x_var + y_var + (x_mean - y_mean) ** 2 + self.eps
        
        ccc = numerator / denominator
        return 1 - ccc.mean()

def VelocityLoss(prediction, target):
    # prediction, target: [batch_size, seq_len, features]

...
class TransformerDenoiser(nn.Module):
    def __init__(self,
                 ...
                 s_latent_embed_drop_prob: float = 0.2,  # speaker_latent_embed 不用
                 s_3dmm_enc_drop_prob: float = 0.2,  # speaker_3dmm_encodings
                 s_emotion_enc_drop_prob: float = 1.0,  # speaker_emotion_encodings
                 past_l_emotion_drop_prob: float = 1.0,  # past_listener_emotion 不用
                 
                 s_future_emotion_enc_drop_prob: float = 0.2, # speaker_future_emotion_prediction
        ...
        self.past_l_emotion_drop_prob = past_l_emotion_drop_prob
         ...
         #新增
        if self.use_stitch:
            # switch_input_dims = [768,58,25]
            # self.stitch_encoder = StitchEncoder(
            #     input_dims=switch_input_dims,
            #     latent_dim=self.latent_dim,
            #     num_layers=stitch_layers,
            #     num_heads=num_heads,
            # )
            switch_input_dims=[768,58,25,25]
            modal_lengths = [60, 60, 60, 10]
            self.stitch_encoder=StitchEncoder(
                input_dims=switch_input_dims,
                latent_dim=self.latent_dim,
                modal_lengths=modal_lengths,
                num_layers=stitch_layers,
                num_heads=num_heads,
            )
        else:# 原有的Proj层
            ...
            # 投影未来说话者特征
            self.future_proj = nn.Linear(s_emotion_dim, latent_dim)
    ...
    def get_model_kwargs(
        ...
        if self.use_stitch:
            ...
            if raw_future is None:
                raw_future = torch.zeros(bs, 10, self.s_emotion_dim).to(sample.device)
            ...
            if speaker_future_emotion_prediction is None:
                ...
                # We can add dropout/masking here if needed, similar to other conditions
        ...
        #     # We can add dropout/masking here if needed, similar to other conditions
        #     speaker_future_emotion_prediction = self.mask_cond(speaker_future_emotion_prediction, mode, self.s_future_emotion_enc_drop_prob)
        #     peaker_future_emotion_prediction = speaker_future_emotion_prediction.permute(1, 0, 2).contiguous()
        ...
        if self.task == "offline":
            ...
            if motion_length is not None:  # during test stage
                ...
                memory_key_padding_mask = torch.zeros(size=(time_embed.shape[1], 0),
                                                      dtype=torch.bool).to(device)
                for emb in emb_latent_list:
                    if emb.shape[0] == mask_u.shape[-1]:#长度为1的
                        memory_key_padding_mask = torch.cat((memory_key_padding_mask, mask_u), dim=-1)
                    elif emb.shape[0] == mask_s.shape[-1]:#长度为60的
                        memory_key_padding_mask = torch.cat((memory_key_padding_mask, mask_s), dim=-1)
                    else:
                        # For other embeddings (like future emotion prediction), assume they are fully valid
                        mask_valid = torch.zeros(size=(emb.shape[1], emb.shape[0]), dtype=torch.bool).to(device)
                        memory_key_padding_mask = torch.cat((memory_key_padding_mask, mask_valid), dim=-1)

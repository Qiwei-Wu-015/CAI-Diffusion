...
class Trainer:
    ...
    def test(self):
        ...
        if os.path.exists('results.pt'):
            logger.info("Found results.pt. Loading cached results...")
            saved = torch.load('results.pt')
            GT_listener_emotions_all = saved['GT']
            pred_listener_emotions_all = saved['PRED']
            input_speaker_emotions_all = []
            
            logger.info("Collecting input speaker emotions from dataloader...")
            for batch_idx, (
                    _, _, speaker_emotion_clips, _, _, _, _, _, _
            ...
            for i in range(math.ceil(all_batch_size / clip_batch_size)):
                ...
                with torch.no_grad():
                    outputs = model(
                        ...
                        speaker_future_emotion_input=None, # In test mode, we don't have future GT, but model will predict it internally
                    )
